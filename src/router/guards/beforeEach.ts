  ...
  try {
    ...
    if (to.path === '/') {
      ...
      if (homePath.value && homePath.value !== '/') {
        console.log('[Router] Перенаправление на главную:', homePath.value)
...
function handleRootPathRedirect(to: RouteLocationNormalized, next: NavigationGuardNext): boolean {
  if (to.path === '/') {
    const { homePath } = useCommon()
    console.log('[Router] handleRootPathRedirect - homePath:', homePath.value)
    if (homePath.value && homePath.value !== '/') {
      console.log('[Router] Перенаправление на:', homePath.value)
      next({ path: homePath.value, replace: true })
      return true
    }
  }
